...
I do not find that any of these partitions substantially improve the utility provided by the equal partition. Using the same dataset for different partitions, I find that the minimum mean squared residual is 11.38 and comes when the weights for the weights for $S_{xx}$, $S_{xy}$, $\bar{x}$, and $\bar{y}$ are 0.30, 0.27,2 0.22, and 0.20, respectively (these do not add to 1 here due to rounding errors). In comparison the minimum mean squared residual for the equal partition was 11.12 and the average was 11.99.
