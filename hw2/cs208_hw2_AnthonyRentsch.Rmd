...
(i) For this mechanism, the minimum value of $\epsilon$ depends on the bounds. Since the data were bounded on [0,1], the minimum value of $\epsilon$ is 0.5, but if $/epsilon$ were on [a,b], then the minimum value is $\frac{b-a}{n}$. From here the value of $\epsilon$ used for the mechanism can be tuned (increased above this minimum value) based on the data owner's standard. 
